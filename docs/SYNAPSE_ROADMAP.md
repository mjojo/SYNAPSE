...
## 🐙 ЭРА 2: ЭВОЛЮЦИЯ (Evolution — Clean Code & New Features)
**Цель:** Рефакторинг кодовой базы + добавление новых фич языка  
**Период:** 2-3 недели  
**Статус:** 🟢 В процессе (Phase 70)  
**Философия:** "Сначала укрепить фундамент, потом строить этажи"

### Phase 70: Documentation First (Карта Магических Чисел) 🗺️ CURRENT

**Цель:** Задокументировать все hardcoded значения перед рефакторингом

**Проблема:** Код полон "магических чисел" (`4194304`, `8232`, `0x1000`), которые делают его нечитаемым и хрупким. Но у нас пока нет глобальных переменных или `const` keyword!

**Решение:** Документировать → Добавить фичу → Рефакторить

**Статус:** ✅ Завершено

| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 70.1 | **Create CONSTANTS.md** | Создать документ со всеми магическими числами и их значениями | ✅ |
| 70.2 | **Categorize Numbers** | Разделить на категории: PE Header, Memory Layout, RVA, IAT | ✅ |
| 70.3 | **Add Context** | Объяснить, откуда взялись значения и почему они критичны | ✅ |
| 70.4 | **Refactoring Roadmap** | Определить порядок действий для Phase 71-72 | ✅ |

**Результат:** `docs/CONSTANTS.md` — полная карта всех констант компилятора  
**Итог:** Теперь мы знаем, что означает каждое число в коде

---

### Phase 71: The Const Keyword (Добавление константов в язык) 🛠️ NEXT

**Цель:** Реализовать поддержку compile-time констант в SYNAPSE

**Почему это проще глобальных переменных:**
- Константы — это compile-time substitution (замена во время парсинга)
- Не требуют выделения памяти в `.bss` или runtime инициализации
- Не нужен Global Scope — просто таблица замен в парсере

**Статус:** ⏳ Запланировано

| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 71.1 | **Lexer: Add `const` keyword** | Добавить новый token type: `TOKEN_CONST` | ⬜ |
| 71.2 | **Parser: Parse const definitions** | `const NAME = VALUE` на верхнем уровне | ⬜ |
| 71.3 | **Symbol Table: Const storage** | Отдельная таблица для констант (имя → значение) | ⬜ |
| 71.4 | **Parser: Substitute in expressions** | При встрече `NAME` в выражении → заменить на `VALUE` | ⬜ |
| 71.5 | **Test: Simple constants** | `const PI = 314` → `let x = PI * 2` | ⬜ |
| 71.6 | **Test: Hex constants** | `const BASE = 0x400000` (поддержка hex literals) | ⬜ |

**Прагматика:** Получаем читаемый код без изменения runtime  
**Пример:**
```synapse
const PE_IMAGE_BASE = 4194304  // 0x400000
const RVA_IAT = 266280         // 0x41028

fn emit_pe_header(state) {
    put_qword(state, PE_IMAGE_BASE)  // Вместо 4194304
    put_dword(state, RVA_IAT)        // Вместо 266280
}
```

**Тест:** Компилятор с константами генерирует байт-в-байт идентичный код с версией без констант

---

### Phase 72: The Great Refactoring (Генеральная Уборка) 🧹

**Цель:** Заменить все магические числа на именованные константы

**Статус:** ⏳ Запланировано (после Phase 71)

| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 72.1 | **Add Const Block** | В начало `synapse_full.syn` — блок из 50+ константных определений | ⬜ |
| 72.2 | **Refactor emit_pe_header** | Заменить все числа на константы | ⬜ |
| 72.3 | **Refactor emit_import_table** | RVA_IAT, RVA_IDATA вместо чисел | ⬜ |
| 72.4 | **Refactor my_alloc** | BSS_ADDR, HEAP_DATA_START вместо чисел | ⬜ |
| 72.5 | **Binary Equivalence Test** | Сравнить Gen N с Gen N+1 → должны быть идентичны | ⬜ |
| 72.6 | **Bootstrap Cycle Verification** | Gen 1 → Gen 2 → Gen 3 → все работают | ⬜ |

**Критерий успеха:**
```bash
bin\synapse.exe examples\synapse_full.syn  # Gen 1
.\synapse_new.exe examples\synapse_full.syn  # Gen 2
fc /b synapse_new.exe out.exe  # Should be IDENTICAL
```

**Итог:** Код читается как документация, но работает так же быстро
